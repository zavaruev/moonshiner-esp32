...
import { z } from "zod";
import { readSensor, readNumber, readTextSensor, readBinarySensor, setNumber, toggleSwitch, pressButton, getAllTemperatures, getAllStatus, } from "./esp32-api.js";
...
// ─── Read tools ───────────────────────────────────────────────
server.tool("read_temperatures", "Read current column and tank temperatures", {}, async () => {
    try {
...
server.tool("get_status", "Get full distillation status: temperatures, process state, alarms, uptime, WiFi, heap", {}, async () => {
    try {
...
server.tool("get_entity", "Read state of any ESPHome entity by its ID (e.g. 'column_temperature', 'target_column_temp', 'status_message')", {
...
// ─── Set / control tools ─────────────────────────────────────
const mkSetTool = (name, desc, entityId, min, max, step) => server.tool(name, desc, { value: z.number().min(min).max(max).describe(`Value (${min}–${max}${step ? `, step ${step}` : ''})`) }, async ({ value }) => {
    try {
...
// ─── Switch tools ────────────────────────────────────────────
const mkSwitchTool = (name, desc, entityId) => server.tool(name, desc, { state: z.boolean().describe("true = on, false = off") }, async ({ state }) => {
    try {
...
// ─── Button tools ────────────────────────────────────────────
server.tool("restart_process", "Restart the distillation process (resets process_finished flag, clears overheat)", {}, async () => {
    try {
